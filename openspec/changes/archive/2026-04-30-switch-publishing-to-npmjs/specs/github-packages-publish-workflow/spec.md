...
**Reason**: `packages: write` was needed to publish to GitHub Packages via the auto-injected `GITHUB_TOKEN`. With publishing moved to npmjs.com (authenticated via `NPM_TOKEN`), the elevated `packages: write` permission is unused. The new minimal-permission requirement is captured in the added "Workflow declares minimal permissions" requirement.

**Migration**: Remove `packages: write` from the workflow's `permissions:` block, leaving only `contents: read`.

### Requirement: Workflow uses the auto-injected `GITHUB_TOKEN`

**Reason**: `GITHUB_TOKEN` only authenticates against GitHub Packages. Publishing to npmjs.com requires an npm token. The new auth requirement is captured in the added "Workflow authenticates via the `NPM_TOKEN` repo secret" requirement.

**Migration**: Replace `NODE_AUTH_TOKEN: ${{ secrets.GITHUB_TOKEN }}` with `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}` in the publish step. Add an `NPM_TOKEN` repo secret holding an npm automation token with publish access to the `@dna-codes` scope.

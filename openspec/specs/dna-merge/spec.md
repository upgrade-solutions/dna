# dna-merge Specification

## Purpose

Defines the pure merge capability for composing multiple Operational DNA documents into a single strand. Lives in `@dna-codes/dna-core` so any package — `dna-ingest` today, future fan-in tools tomorrow — can compose DNA chunks without depending on the orchestrator. The function is deterministic and side-effect free: identity is by `name` within primitive type; list-shaped child fields union by name; scalar conflicts surface as typed `Conflict` entries each carrying an explicit `recommendation` (rather than silently last-writer-wins'ing); cross-references resolve against the merged noun set with unresolved refs reported as warnings rather than dropped. A separate provenance map keyed by dotted primitive path tracks which source(s) contributed each primitive, so conflicts can cite specific documents.

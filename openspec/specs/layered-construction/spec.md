# layered-construction Specification

## Purpose

`@dna-codes/input-text` supports two construction modes for converting prose into Operational DNA: a one-shot mode that asks the LLM for a single JSON document (the original behavior), and a **layered** mode that drives the LLM through tool calls — one primitive at a time — with per-call schema and reference-integrity checks. Layered mode trades token volume for reliability: a bad call recovers locally instead of throwing the whole document away, which makes smaller local models viable on real-world transcripts.

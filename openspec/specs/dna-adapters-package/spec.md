# dna-adapters-package Specification

## Purpose

Defines the unified adapter package — `@dna-codes/dna-adapters` — that hosts every input, output, and integration adapter as a single workspace with subpath exports. Adapters live as self-contained, mechanically extractable folders under one package, one version, and one publish, while `@dna-codes/dna-ingest` remains a separate package that depends on no adapters.

...
| **Domain Event** | `Signal` (when cross-domain) + `Outcome.emits` (the publish point) | DDD's domain events fire when the model changes meaningfully. DNA models them as `Signal` (with a typed payload contract, fully-qualified name) emitted from an `Outcome`. Signals cross Domain boundaries. Intra-domain consequences use `Outcome.changes` (state) and `Outcome.initiates` (chained Operations). |
...
| **Context Map** (Shared Kernel, Customer/Supplier, Conformist, etc.) | Documented relationships between Domains; signal contracts | DNA doesn't formalize context-map relationships; in practice they're expressed as which Signals one Domain consumes from another. |

...
| [Domain-Driven Design](./ddd.md) | Tactical + strategic domain modeling in code | Closest in spirit; Bounded Context = `Domain`, Aggregate Root = `Resource`, Domain Event = `Outcome.changes` (no first-class event primitive in DNA today), Specification = condition `Rule` |
...
| [Event Storming](./event-storming.md) | Workshop output → DNA translation | Sticky-color → primitive mapping. Aggregate (tan) = `Resource`; Command (blue) = `Operation`; Event (orange) maps to `Outcome.changes` (DNA has no first-class event primitive today); Policy (purple) = `Trigger`; Actor (yellow) = `Resource`-as-Role |

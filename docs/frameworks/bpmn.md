...
| **Script Task** | Not modeled in Operational DNA | DNA does not currently have a named-computation primitive. Computational steps are typically modeled as Operations whose Outcomes mutate Resource attributes, with the actual implementation deferred to a downstream cell. |
...
| **Intermediate Throw Event** | Not modeled | DNA does not currently emit first-class events mid-Process. State changes are captured in the Step's Operation's `Outcome.changes`. |

  ...
  for (const key of keys) {
    ...
    if (v === null || v === undefined) {
      ...
      if (innerKeys.length === 0) {
        ...
        lines.push(keyStr)
        lines.push(emitObject(v as Record<string, unknown>, indent + 1).trimEnd())
      }
    } else {
...
function emitArrayBody(arr: unknown[], indent: number): string {
  const pad = '  '.repeat(indent)
  const lines: string[] = []
  for (const item of arr) {
    if (item === null || item === undefined) {
      ...
      const innerKeys = Object.keys(item as object)
      if (innerKeys.length === 0) {
        lines.push(`${pad}- {}`)
      } else {
        const inner = emitObject(item as Record<string, unknown>, indent + 1).trimEnd()
...
function quoteKey(key: string): string {
  if (PLAIN_KEY.test(key)) return key
  return quote(key)
}

...
const schema_to_tool_1 = require("./schema-to-tool");
describe('schema-to-tool', () => {
    it('builds one tool per Operational primitive plus finalize', () => {
        const tools = (0, schema_to_tool_1.buildLayeredTools)();
    ...
    it('inlines $refs to attribute and action sub-schemas', () => {
        const tool = (0, schema_to_tool_1.buildPrimitiveTool)('resource');
    ...
    it('strips $id and $schema metadata from inlined schemas', () => {
        for (const kind of schema_to_tool_1.PRIMITIVE_KINDS) {
            const tool = (0, schema_to_tool_1.buildPrimitiveTool)(kind);
    ...
    it('injectEnums narrows membership.role to declared role names', () => {
        const tools = (0, schema_to_tool_1.buildLayeredTools)();
        const narrowed = (0, schema_to_tool_1.injectEnums)(tools, { roles: ['Underwriter', 'LoanOfficer'] });
        const membership = narrowed.find((t) => t.name === 'add_membership');
        const props = membership.parameters.properties;
        expect(props.role.enum).toEqual(['Underwriter', 'LoanOfficer']);
    });
    it('injectEnums leaves fields alone when the pool is empty', () => {
        const tools = (0, schema_to_tool_1.buildLayeredTools)();
        const narrowed = (0, schema_to_tool_1.injectEnums)(tools, {});
        const membership = narrowed.find((t) => t.name === 'add_membership');
        const props = membership.parameters.properties;
    ...
    it('injectEnums does not mutate the source tool list', () => {
        ...
        (0, schema_to_tool_1.injectEnums)(tools, { roles: ['Underwriter'] });
        expect(JSON.stringify(tools)).toBe(before);
    });
});
//# sourceMappingURL=schema-to-tool.test.js.map
...
describe('client — Integration.write', () => {
    it('jira:child://<EPIC> creates a child issue with summary + dna label + epic links', async () => {
        ...
        const { fetchImpl } = mockFetch((url, init) => {
            bodies.push({ url, payload: JSON.parse(init.body) });
            return { body: { id: '7', key: 'ENG-107', self: '' } };
        });
        const client = (0, client_1.createClient)({ ...baseOpts, fetchImpl });
        const result = await client.write('jira:child://ENG-100?summary=Apply+Loan&label=operation-loan-apply', { contents: 'As a Borrower\n\nI want to submit', mimeType: 'text/markdown' });
        expect(result.target).toBe('jira://ENG-107');
        expect(bodies).toHaveLength(1);
        const fields = bodies[0].payload.fields;
    ...
    it('jira:child://<EPIC> appends extra-label query params to the labels array', async () => {
        ...
        const client = (0, client_1.createClient)({ ...baseOpts, fetchImpl });
        await client.write('jira:child://ENG-100?summary=X&label=op-x&extra-label=generated&extra-label=needs-review', { contents: 'body', mimeType: 'text/markdown' });
        const fields = bodies[0].fields;
        expect(fields.labels).toEqual(['dna:op-x', 'generated', 'needs-review']);
    });
    it('jira:child:// rejects writes without summary', async () => {
        const { fetchImpl } = mockFetch(() => ({ body: {} }));
        const client = (0, client_1.createClient)({ ...baseOpts, fetchImpl });
        await expect(client.write('jira:child://ENG-100', { contents: 'b', mimeType: 'text/markdown' })).rejects.toThrow(/summary/);
    });
    it('jira://<KEY> updates the named issue, preserving non-dna labels', async () => {
        ...
        const client = (0, client_1.createClient)({ ...baseOpts, fetchImpl });
        const result = await client.write('jira://ENG-107?summary=New+Title&label=operation-loan-apply', { contents: 'updated body', mimeType: 'text/markdown' });
        expect(result.target).toBe('jira://ENG-107');
        const put = seen.find((s) => s.method === 'PUT');
        const fields = put.body.fields;
    ...
    it('rejects non-markdown payloads', async () => {
        const { fetchImpl } = mockFetch(() => ({ body: {} }));
        const client = (0, client_1.createClient)({ ...baseOpts, fetchImpl });
        await expect(client.write('jira:child://ENG-100?summary=X&label=y', {
            contents: '{}',

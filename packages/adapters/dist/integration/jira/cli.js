...
const text_1 = require("@dna-codes/dna-adapters/input/text");
const text_2 = require("@dna-codes/dna-adapters/output/text");
...
async function pushCommand(args, env) {
    ...
    const docs = (0, text_2.renderMany)(dna, { styles: { operation: style ?? 'user-story' } });
    if (dryRun) {
...
async function syncCommand(args, env) {
    ...
    const docs = (0, text_2.renderMany)(dna, { styles: { operation: 'user-story' } });
    const created = [];
    for (const doc of docs) {
...
function buildWriteUri(mode, key, summary, label, extraLabels) {
    const params = new URLSearchParams();
    params.set('summary', summary);
    params.set('label', label);
    for (const l of extraLabels)
        params.append('extra-label', l);

...
interface ResourceShape {
  ...
  parent?: string
  scope?: string
  memberships?: { role: string; in: string }[]
  actions?: { name: string }[]
...
interface OperationalDNA {
  ...
  operations?: { name: string; resource: string; action: string }[]
...
export class DnaValidator {
  ...
  private collectResources(domain: OperationalDNA['domain']): ResourceShape[] {
    const resources: ResourceShape[] = [...(domain.resources ?? [])]

...
export interface PushStoriesResult {
  epicKey: string
  created: Array<{ id: string; key: string; summary: string }>
  /** Populated only when `dryRun` is true. */
  planned?: Array<{ summary: string; description: string }>
}

export interface UpdateStoriesResult {
  epicKey: string
  updated: Array<{ key: string; summary: string }>

...
describe('mapping — dnaToStoryFields', () => {
  it('creates one Story-shaped field set per capability', () => {
    const stories = dnaToStoryFields(dnaFixture, {
      projectKey: 'ENG',
      epicKey: 'ENG-100',
      issueType: 'Story',
    })
  ...
  it('merges extra labels', () => {
    const [story] = dnaToStoryFields(dnaFixture, {
      projectKey: 'ENG',
      epicKey: 'ENG-100',
      issueType: 'Story',
...
describe('client', () => {
  ...
  it('pushStoriesToEpic creates one issue per capability with ADF description', async () => {
    ...
    const first = bodies[0] as { fields: { description: { type: string } } }

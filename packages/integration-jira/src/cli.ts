...
async function pushCommand(args: ArgMap, env: NodeJS.ProcessEnv): Promise<number> {
  ...
  if (dryRun) {
    ...
    for (const p of result.planned ?? []) console.log(`- ${p.summary}`)
  } else {
    console.log(`created ${result.created.length} stor(ies) under ${epic}`)
    for (const c of result.created) console.log(`- ${c.key} — ${c.summary}`)
  }
  return 0
}
...
async function updateCommand(args: ArgMap, env: NodeJS.ProcessEnv): Promise<number> {
  ...
  for (const u of result.updated) console.log(`- ${u.key} — ${u.summary}`)
...
function pullOptionsFromEnv(env: NodeJS.ProcessEnv): PullEpicOptions {
  const provider = (env.DNA_LLM_PROVIDER ?? 'anthropic') as PullEpicOptions['provider']

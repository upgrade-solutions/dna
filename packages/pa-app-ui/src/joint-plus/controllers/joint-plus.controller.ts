...
export class JointPlusController extends Controller {
    ...
    startListening() {
        ...
        const { graph, paper, toolbar, history, eventBusService, scroller } = this.service;
        ...
        // Add mouse wheel zoom with Cmd/Ctrl key
        scroller.el.addEventListener('wheel', (evt: WheelEvent) => onMouseWheel(this.service, evt), { passive: false });
    }
}
...
function onMouseWheel(service: JointPlusService, evt: WheelEvent): void {
    ...
    if (evt.metaKey || evt.ctrlKey) {
        ...
        const zoom = delta * ZOOM_STEP;
        
        // Get mouse position relative to the paper
        const scrollerRect = scroller.el.getBoundingClientRect();
        const ox = evt.clientX - scrollerRect.left;
        const oy = evt.clientY - scrollerRect.top;
        
        scroller.zoom(zoom, { min: ZOOM_MIN, max: ZOOM_MAX, ox, oy });
    }
}

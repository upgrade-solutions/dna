...
export const init = () => {
    ...
    assumptionBackgrounds.forEach((rect) => rect.appendTo(paper.getLayerNode(dia.Paper.Layers.BACK)));
    assumptionLabels.forEach((label) => label.appendTo(paper.getLayerNode(dia.Paper.Layers.BACK)));
    stackLabels.forEach((label) => label.appendTo(paper.getLayerNode(dia.Paper.Layers.BACK)));
    ...
    // Update the cells in the graph based on the new data (encoded in the cells)
    function updateGraph(graph, cells) {
        ...
        cells.forEach(cell => {
            ...
            if (originalCell) {
                if (originalCell.get('type') !== cell.get('type')) {
                    ...
                    originalCell.set(cell.attributes);
                }
            }
            else {
                graph.addCell(cell);
    ...
    function buildWithCurrentOptions() {
        updateGraph(graph, buildTheoryOfChange(theoryOfChange, {

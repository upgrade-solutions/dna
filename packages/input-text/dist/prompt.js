...
const LAYER_GUIDE = {
    operational: [
        ...
        '    — a SUBJECT (target) is one of four noun primitives: Resource (structure — Loan, Invoice, Post), Person (individual — Patient, Borrower, Employee), Role (position — Underwriter, Doctor), or Group (work-unit/container — BankDepartment, Hospital, Case).',
        ...
        '    — Resources: structure templates the org manages (Loan, Account, Document, Invoice). Have name, attributes, actions, optional parent.',
...
function buildSystemPrompt(layers, instructions) {
    ...
    if (layers.includes('operational')) {
        lines.push('', 'Structural skeleton — operational layer. Treat <placeholder> tokens as slots to fill from the input text; never emit them literally:', '', OPERATIONAL_SKELETON);
    }
    if (layers.includes('product')) {
        lines.push('', 'Structural skeleton — product layer (core / api / ui). Include only sub-sections the text describes; omit the layer entirely if no software is named:', '', PRODUCT_SKELETON);
    }
    if (layers.includes('technical')) {
        lines.push('', 'Structural skeleton — technical layer. Include only if the text names deployment details; omit the layer entirely otherwise:', '', TECHNICAL_SKELETON);
    }
    if (instructions) {

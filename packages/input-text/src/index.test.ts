...
function mockFetch(responseBody: unknown, status = 200): { fetchImpl: typeof fetch; calls: FetchArgs[] } {
...
describe('@dna-codes/input-text', () => {
  describe('dispatch', () => {
    it('sends an OpenAI chat completion request with Bearer auth', async () => {
      ...
      await parse('A lending business.', {
        provider: 'openai',
        apiKey: 'sk-test',
        fetchImpl,
      })
  ...
  describe('response parsing', () => {
    ...
    it('splays out product and technical layers when present', async () => {
      ...
      const result = await parse('Full system.', {
        provider: 'openai',
        apiKey: 'sk-test',
        fetchImpl,
      })
    ...
    it('throws on invalid JSON with the raw text included', async () => {
      ...
      await expect(
        parse('x', { provider: 'openai', apiKey: 'sk-test', fetchImpl }),
  ...
  describe('validation', () => {
    it('rejects empty text', async () => {
      await expect(
        parse('', { provider: 'openai', apiKey: 'sk-test', fetchImpl: (() => {}) as unknown as typeof fetch }),
      ).rejects.toThrow(/non-empty string/)
    })

    it('rejects missing api key', async () => {
      await expect(
        parse('x', { provider: 'openai', apiKey: '', fetchImpl: (() => {}) as unknown as typeof fetch }),
